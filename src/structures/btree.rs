...
use crate::types::{
    decode_utf16be, decode_utf16le, utf16_length_prefix_bytes, utf8_length_prefix_bytes, DataType,
};
...
pub(crate) struct FixedSizeComparator(pub usize);
...
impl TryFrom<&DataType> for FixedSizeComparator {
    type Error = ();

    fn try_from(data_type: &DataType) -> Result<Self, Self::Error> {
        match data_type {
            DataType::Varchar(_) | DataType::Bool | DataType::Text | DataType::Blob => Err(()),
            fixed => Ok(Self(fixed)),
        }
    }
}

/// Compares variable-length data (Varlena types).
///
/// Format expected:
/// ```text
/// self.0 bytes
...
/// V           V
/// +-----------+--------+--------+--------+
/// | DATA LEN  | BYTE 0 | BYTE 1 |  ...   |
/// +-----------+--------+--------+--------+
/// ^           ^                          ^
/// |           +------ DATA LEN bytes ----+
/// |
/// +-- Length prefix (self.0 bytes)
/// ```
#[derive(Debug, Clone, Copy, PartialEq)]
...
impl VarlenaCmp {
    pub fn new(prefix_bytes: usize, encoding: Option<TextEncoding>) -> Self {
        Self {
            prefix_bytes,
            encoding,
        }
    ...
    pub fn for_string(max_characters: usize, encoding: TextEncoding) -> Self {
        let prefix_bytes = match encoding {
            TextEncoding::Utf8 => utf8_length_prefix_bytes(max_characters),
            TextEncoding::Utf16le | TextEncoding::Utf16be => {
                utf16_length_prefix_bytes(max_characters)
            }
        };
        Self {
            prefix_bytes,
    ...
    pub fn for_blob(max_size: usize) -> Self {
        let prefix_bytes = match max_size {
            0..=255 => 1,
            256..=65535 => 2,
            _ => 4,
        };
...
impl BytesCmp for VarlenaCmp {
    fn bytes_cmp(&self, a: &[u8], b: &[u8]) -> Ordering {
        // Leer longitudes
        let len_a = read_length(&a[..self.prefix_bytes], self.prefix_bytes);
        let len_b = read_length(&b[..self.prefix_bytes], self.prefix_bytes);

        let data_a = &a[self.prefix_bytes..self.prefix_bytes + len_a];
        let data_b = &b[self.prefix_bytes..self.prefix_bytes + len_b];
        ...
        match self.encoding {
            ...
            Some(TextEncoding::Utf16le) => {
                let str_a = decode_utf16le(data_a);
                let str_b = decode_utf16le(data_b);
                str_a.cmp(&str_b)
            }
            Some(TextEncoding::Utf16be) => {
                let str_a = decode_utf16be(data_a);
                let str_b = decode_utf16be(data_b);
                str_a.cmp(&str_b)
            }
            None => {
                // Comparación binaria para blobs
                data_a.cmp(data_b)
            }
        }
    }
...
fn read_length(bytes: &[u8], len: usize) -> usize {
    let mut buf = [0u8; 8];
...
// Actualizar el enum para incluir VarlenaCmp
#[derive(Debug, Clone, Copy, PartialEq)]
    ...
    fn from(data_type: &DataType) -> Self {
        match data_type {
            DataType::Varchar(max_chars) => {
                // Asumiendo UTF-8 por defecto, ajustar según necesidad
                Self::VarlenaCmp(VarlenaCmp::for_string(*max_chars, TextEncoding::Utf8))
            }
            DataType::Blob(max_size) => Self::VarlenaCmp(VarlenaCmp::for_blob(*max_size)),

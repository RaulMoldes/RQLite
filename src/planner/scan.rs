...
use crate::structures::bplustree::{BPlusTree, NodeAccessMode, IterDirection,
    Payload,Comparator};
...
where T: Comparator
{
    fn exec(&mut self, db: &mut Database) -> std::io::Result<ResultSet> {
        match self {
            ...
            Self::IndexScan { table, index, index_schema, start, end } => {
                let mut collected = ResultSet::new();
                            ...
                            Ok(payload) => {
                                ...
                                match index.compare(payload.as_ref(), end)? {
                                    std::cmp::Ordering::Equal => {
                                        ...
                                        let result = table.search_from_root(value.as_ref(), NodeAccessMode::Read)?;
                                        if let Some(payload_table) = table.get_content_from_result(result){

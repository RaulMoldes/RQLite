...
mod test_helpers {
    ...
    pub(crate) use crate::storage::core::traits::{Buffer, BufferOps, BtreeBuffer, WalBuffer, Writable};
    pub(crate) use crate::storage::cell::{OwnedCell, Slot, CELL_HEADER_SIZE};
    pub(crate) use crate::storage::page::{
        BtreePage, OverflowPage, BtreePageHeader,
        PageZeroHeader, OverflowPageHeader,
    };
    pub(crate) use crate::storage::wal::{
        WalBlock, BlockZero, OwnedRecord, RecordType,
        BlockHeader, BlockZeroHeader, WAL_BLOCK_SIZE,
    };
    pub(crate) use crate::types::{TransactionId, ObjectId, PageId};
}
...
fn test_memblock_non_null_roundtrip<H>() {
    ...
    drop(unsafe {
        MemBlock::<H>::from_non_null(b.into_non_null())
    });
}
...
fn test_page_alloc(size: usize) {
    ...
    let p = BtreePage::alloc(size as u32);
    assert!(p.is_empty());
    assert!(p.is_leaf());
}
...
fn test_page_multi(size: usize) {
    ...
    let mut p = BtreePage::alloc(size as u32);
    for i in 0..10u8 {
...
test_suite!(pg_stress {
    sequential => {
        use super::test_helpers::*;
        let mut p = BtreePage::alloc(8192);
        for i in 0u32..50 {
    ...
    fill_refill => {
        use super::test_helpers::*;
        let mut p = BtreePage::alloc(4096);
...
test_suite!(overflow_pg {
    alloc => {
        use super::test_helpers::*;
        assert!(OverflowPage::alloc(4096).next().is_none());
    },
    to_overflow => {
        use super::test_helpers::*;
        let mut p = BtreePage::alloc(4096);
...
fn test_wal_alloc_drop() {
    let alloc = WalBlock::alloc(WAL_BLOCK_SIZE as u32);
    drop(alloc);
}
...
test_suite!(wal_misc {
    fill_read => {
        use super::test_helpers::*;
        let mut b = WalBlock::alloc(WAL_BLOCK_SIZE as u32);
        let mut off = 0usize;
